...
def get_data(
		...
		ignore_accumulated_values_for_fy=False , total = True, parent_account=None, total_name=None):

	accounts = get_accounts(company, root_type, parent_account)
	my_account = my_accounts(company, parent_account)
	# my_accounts=["Direct Revenue - DMCC","Cost of Sales - DMCC"]
	...
	calculate_values(
		accounts_by_name, gl_entries_by_account, period_list, accumulated_values, ignore_accumulated_values_for_fy,my_account)
	accumulate_values_into_parents(accounts, accounts_by_name, period_list, accumulated_values)
	out = prepare_data(accounts, balance_must_be, period_list, company_currency,my_account,parent_account)
	out = filter_out_zero_value_rows(out, parent_children_map,False,my_account)

	if out and total:
		add_total_row(out, total_name, period_list, company_currency,parent_account)
...
def calculate_values(
		accounts_by_name, gl_entries_by_account, period_list, accumulated_values, ignore_accumulated_values_for_fy,my_accounts):
			...
			for period in period_list:
				...
				if entry.posting_date <= period.to_date:
					if (accumulated_values or entry.posting_date >= period.from_date) and \
						...
						# if entry.account != my_accounts[0]:
						# 	d[period.key] = 5.0
						# else:
...
def prepare_data(accounts, balance_must_be, period_list, company_currency, my_accounts=None, parent_account=None):
	data = []
...
def filter_out_zero_value_rows(data, parent_children_map, show_zero_values=False, my_accounts=[]):
	data_with_value = []
	for d in data:
...
def my_accounts(company, parent_account=None):
	mylist=[]
	...
	if lft:
		data= frappe.db.sql("""
			select name
			from `tabAccount`
			where company=%s and lft >= %s and rgt <= %s order by lft
			""", (company, lft, rgt), as_dict=True)
		if data:
			for d in data:
				mylist.append(d.name)
	return mylist

def add_total_row(out, root_type, period_list, company_currency,parent_account):
	total_row = {
	...
	for row in out:
		if row.get("account") == parent_account:
			for period in period_list:
...
def get_accounts(company, root_type, parent_account=None):
	...
	# parent_type = ""
	# if root_type=="Income":
	# 	parent_type = frappe.db.get_value("Account", {"company": company, "account_name": "Revenue"}, "name")
	# else:
	# 	parent_type = frappe.db.get_value("Account", {"company": company, "account_name": "Expenses"}, "name")
	# if lft:
	# 	return frappe.db.sql("""
	# 		select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
	# 		from `tabAccount`
	# 		where company=%s and root_type=%s and ((lft >= %s and rgt <= %s) or name = %s) order by lft
	# 		""", (company, root_type, lft, rgt,parent_type), as_dict=True)
	# else:
	return frappe.db.sql("""
		select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
		from `tabAccount`
